<template>
  ...
  export default {
    ...
    setup() {
      ...
      const attack = () => {
        ...
        logs.value.push(`Herói atacou e causou ${heroDamage} de dano.`);
      ...
      const defend = () => {
        logs.value.push('Herói defendeu.');
        villainAttack(true);
      };
  
      const usePotion = () => {
        heroHealth.value = Math.min(heroHealth.value + 50, 100);
        logs.value.push('Herói usou uma poção e recuperou 50 de vida.');
        villainAttack();
      };
  
      const runAway = () => {
        logs.value.push('Herói tentou correr, mas o vilão atacou.');
        villainAttack();
      };
  
      const villainAttack = (defending = false) => {
        const villainDamage = getRandomDamage();
        const damageTaken = defending ? Math.floor(villainDamage / 2) : villainDamage;
  ...
  .hero-container, .villain-container {
    width: 30%;
    text-align: center;
  }
